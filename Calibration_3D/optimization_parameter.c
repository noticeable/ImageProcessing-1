...
int main(void)
{
    ...
    i = 1;
    while (fabs(dfpa(x)) > epsilon && fabs(dfpb(x)) > epsilon && fabs(dfpg(x)) > epsilon && fabs(dfpcx(x)) > epsilon && fabs(dfpcy(x)) > epsilon && fabs(dfpcz(x)) > epsilon)
    {
         // ベクトルxの更新
        update_x(x, eta);
        i++;
...
double fp(double x[])
{
    ...
    // 左カメラの3軸を回転、平行移動
    x_ = a*x_w + b*y_w + c*z_w - x[3]*a - x[4]*b - x[5]*c + x[3];
    y_ = d*x_w + e*y_w + f*z_w - x[3]*d - x[4]*e - x[5]*f + x[4]; 
    z_ = g*x_w + h*y_w + i*z_w - x[3]*g - x[4]*h - x[5]*i + x[5];
    ...
    s = (coef_c[0]*x_w + coef_c[1]*y_w + coef_c[2]*z_w + coef_c[3]) / (coef_c[8]*x_w + coef_c[9]*y_w + coef_c[10]*z_w + 1);
    t = (coef_c[4]*x_w + coef_c[5]*y_w + coef_c[6]*z_w + coef_c[7]) / (coef_c[8]*x_w + coef_c[9]*y_w + coef_c[10]*z_w + 1);
